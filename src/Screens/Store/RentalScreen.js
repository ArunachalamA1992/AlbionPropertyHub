...
const RentalScreen = ({route, navigation}) => {
  ...
  const [filterRentData] = useState(route.params.data);
  const [filterBoolean] = useState(route.params.filter);
            ...
            {val?.features != undefined && (
              <Text
                ...
                <Text
                  ...
                  {' '}
                  {common_fn.formatText(furnish_status)}
                </Text>
              </Text>
            )}
          </View>
  ...
  const loadMoreData = async () => {
    ...
    try {
      ...
      if (filterBoolean) {
        var filterdata = filterRentData + '&page_number=' + nextPage;
        const filterData = await fetchData.FilterData(filterdata);
        if (filterData.length > 0) {
          setPage(nextPage);
          const updatedData = [...rentData, ...filterData];
        ...
        if (response.length > 0) {
          setPage(nextPage);
          const updatedData = [...rentData, ...response];
          setRentData(updatedData);
          // await AsyncStorage.setItem('rentData', JSON.stringify(updatedData));
        } else {
          setEndReached(true);
        ...
        <>
          ...
          <View
            ...
            <Text
              ...
              {countFilter ?? 0} results | Properties for Rent in {location}
            </Text>
          </View>
          <FlatList
            data={final}
